...
def change_status(defect: Defect, new_status: str, actor: User):
    ...
    if actor.is_engineer:
        if not (current == Defect.STATUS_IN_PROGRESS and new_status == Defect.STATUS_REVIEW):
            raise PermissionError("Инженер может только переводить 'В работе' -> 'На проверке'")
